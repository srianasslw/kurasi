...
if not st.session_state["logged_in"]:
    ...
    with st.expander("📢 BACA PANDUAN FORMAT EXCEL (PENTING)"):
        ...
        4. **Kriteria Lolos**: NIK berjumlah **16 digit** angka dan **tidak ganda**.
        5. **Format Tanggal**: Otomatis disesuaikan menjadi dd/mm/yyyy.
        """)
        ...
        with st.spinner('⏳ Sedang memvalidasi dan memilah data...'):
            ...
            if nik_col:
               ...
               # --- 4. DETAIL VERIFIKASI ANOMALI ---
                ...
                with c1:
                    st.download_button("✅ Unduh Data Lolos", data=to_excel(df_lolos), file_name=f"1_LOLOS_{role_user}_{uploaded_file.name}")
                with c2:
                    if not df_luar_wilayah.empty:
                        st.download_button("📍 Unduh Data Luar Wilayah", data=to_excel(df_luar_wilayah), file_name=f"2_3277+_{role_user}_{uploaded_file.name}")
                with c3:
                    if not df_anomali.empty:
                        st.download_button("📝 Unduh Data Anomali", data=to_excel(df_anomali), file_name=f"3_REVISI_{role_user}_{uploaded_file.name}")
                with c4:
                    if not df_ganda.empty:
                        st.download_button("🗑️ Unduh Data Ganda", data=to_excel(df_ganda), file_name=f"4_GANDA_{role_user}_{uploaded_file.name}")
            else:
